...
class AddRedCard extends StatefulWidget {
  final MatchDataModel matchData;
  final bool isHomeTeam;

  const AddRedCard({Key key, this.matchData, this.isHomeTeam})
      : super(key: key);

  @override
  _AddRedCardState createState() => _AddRedCardState();
}

class _AddRedCardState extends State<AddRedCard> {
...
class _MyPlayerListTileState extends State<MyPlayerListTile> {
  ...
  void initState() {
    playerScore = widget.isHomeTeam
        ? widget.matchData
            .redCardsT1[(widget.playerList[widget.index].id).toString()]
        : widget.matchData
            .redCardsT2[(widget.playerList[widget.index].id).toString()];
    ...
    playerScores = widget.isHomeTeam
        ? widget.matchData.redCardsT1
        : widget.matchData.redCardsT2;
    super.initState();
  }
                    ...
                    onTap: () async {
                      ...
                      if (widget.isHomeTeam) {
                        ...
                        await matchesRef.doc(widget.matchData.matchId).update({
                          'redCardsCountT1':
                              widget.matchData.redCardsCountT1 + difference,
                          'redCardsT1': playerScores,
                        ...
                        await matchesRef.doc(widget.matchData.matchId).update({
                          'redCardsCountT2':
                              widget.matchData.redCardsCountT2 + difference,
                          'redCardsT2': playerScores,
                          'recentGoals': recentGoals
                        });
                      }
                      setState(() {
